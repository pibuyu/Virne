...
METRIC_COL = "long_term_time_r2c_ratio"
METRIC_SLUG = "long_term_time_r2c"


def main():
    parser = argparse.ArgumentParser(description="Plot long-term time r2c ratio over time")
    ...
    if METRIC_COL not in df.columns:
        if "total_time_revenue" in df.columns and "total_time_cost" in df.columns:
            df[METRIC_COL] = df["total_time_revenue"] / df["total_time_cost"].replace(0, np.nan)
        else:
    ...
    title = args.title or "Long-term Time R2C Ratio"
